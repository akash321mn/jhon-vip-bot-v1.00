        ...
        try:
            ...
            if member.status not in ["member", "administrator", "creator"]:
                ...
                await update.message.reply_text(
                    ...
                    f"🚀 Join now to unlock premium signals!",
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❌ Error checking channel membership. Contact admin.")
        return ConversationHandler.END
        ...
        if member.status in ["member", "administrator", "creator"]:
            ...
            if is_authenticated(user_id):
                ...
                await context.bot.send_message(
                    ...
                    text=(
                        "✅ Channel verified!\n\n"
                        "🔐 Now authenticate to continue.\n\n"
                        f"Contact {OWNER_CONTACT} for credentials.\n\n"
                        "Please enter your USERNAME:"
                    )
                )
                # Note: This won't work perfectly without conversation state
                # User should use /start again or we need to manage state differently
        else:
            await query.answer("❌ You haven't joined yet!", show_alert=True)
    except Exception as e:
        logger.error(f"Verify error: {e}")
        await query.answer("❌ Verification failed. Try again.", show_alert=True)
